...
network containing only activation layers: $\mathcal{F}=F_{\mathrm{ReLU}} \circ F_{\math}$
...
\begin{equation}
    \mathcal{L}(\mathcal{F})^2 = \sum_{i} |\mathcal{F}(x)_i - y_i|^2
\end{equation}
$L_2$ is a universal Loss function, which is used in cases where the label space $\mathbb{Y}$ is continuous without further interpretation (e.g. floorspace and height of a house, based on the price: \mathbb{R}^1 \rightarrow \mathbb{R}^2).
