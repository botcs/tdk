...
That encourages us to derive our differentials on paper first, and use \emph{numerical gradient approximation} for checking whether our implementation is correct.
...
We can apply the same method for each variable, the result will be the \emph{gradient} or $$
\begin{equation}
...
We will see that these rules plays a very fundamental role in training networks, especially when taking outer products or convolving.