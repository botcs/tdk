...
&= -y^*_i-y^*_i p_i + \sum_{j \neq i} y^*_j p_i = -y^*_i + \sum_j y^*_j p_i \\
&= p_i \left(\sum_j y^*_j\right) -y^*_i = p_i - y^*_i
\end{split}
\end{equation}
The trick of the derivation is that $y^*$ has only one non-zero element, and the lemma:
$$
